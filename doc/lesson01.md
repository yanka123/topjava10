...
- 1.2 Сделать отображения списка еды в jsp, цвет записи в таблице зависит от параметра `isExceeded` (красный/зеленый).
  - 1.2.1 Список еды захардкодить (желательно без дублирования в коде)
...
- 2.2 Работать с реализацией через интерфейс, который не должен ничего знать о деталях реализации (Map, DB или что-то еще).
...
- 5 Работать с едой надо через интерфейс. Реализаций хранения будет несколько, нужно учитывать это в названии класса имплементации работы в памяти.
